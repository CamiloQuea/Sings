...
export class NotesMap {
  ...
  constructor(canvas: HTMLCanvasElement, song: Song, fps: number) {
    ...
    this.virtualCanvas.resize({
      width: this.virtualCanvas.canvas.width + pixelCount,
      height: this.virtualCanvas.canvas.height,
    });
  ...
  generateBackground(): string {
    ...
    this.virtualCanvasCtx.fillRect(
      0,
      0,
  ...
  placeNote(index: number, widhtShift: number) {
    ...
    const heightPos =
      canvasHeightDrawable -
      (notePercentHeight / this.song.freqDiff) * canvasHeightDrawable;
